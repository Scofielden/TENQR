function [VaR, Table_coeff] = CAViaROptimisation_gjw_final(MODEL, THETA, dataCAViaR)
%参考https://cn.mathworks.com/matlabcentral/answers/135278-question-about-optimset-option-linesearchtype
%修改日期 20160919
% ****************************************************************************************************************************************
% *                                                                                                                                      *
...
%load H:\Matlab7\CAViaRCodes\data\dataCAViaR.txt 
% *****************************************************************************************
...
inSampleObs = size(ytot,1);				    % Number of in sample observations.
...
% options = optimset('LargeScale', 'off', 'HessUpdate', 'dfp', 'LineSearchType', 'quadcubic','MaxFunEvals', MaxFunEvals, ...
 ...
 options =optimset('TolFun', 1e-8, 'TolX', 1e-8,'MaxFunEvals', MaxFunEvals, ...
...
for t = 1:nSamples
   ysort(:, t)          = sortrows(y(1:52, t), 1); 
   empiricalQuantile(t) = ysort(round(52*THETA), t);
end
...
% Table_coeff=[BetaHat(1,:); standardErrors(1,:); coeffPvalue(1,:);
